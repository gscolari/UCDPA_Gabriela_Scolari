...
# Here my analysis starts# Analysing how many women have the intention or not to avoid pregnancy grouped by “continent” and “pregnancy intention” # The groupby function was the one that best suited this part of analysis since I wanted to know the difference amongst continents. 
# In[20]:
...
# Analysing how many women want to avoid pregnancy related to having available contraceptive to meet their needs. # Calculating value counts for "Pregnancy intention" and "Contraceptive availability" 
# In[21]:
...
#This was my single analysis for “contraceptive” Data Frame. #Now I will start analysing my second Data Frame, “violence” - This data was taken from a survey of men and women in African, Asian, and South American countries, exploring the attitudes and perceived justifications given for committing acts of violence against women. The data also explores different sociodemographic groups that the respondents belong to, including: Education Level, Marital status, Employment, and Age group. # My first analysis is to have an overview of how many people with ages "15-24", "25-34", "35-49" years old participate of this survey. # Filtered the Data Frame based on "Demographics Question" containing "age" 
# In[26]:
...
# Then I analysed the country with highest value (most violent country for women) and the lowest value (least violent country for women): # Finding the country with the highest value 
# In[29]:
...
#Then I analysed what country would be the middle term for women to live in, not the most violent neither the least violent # Calculating the median of the "Value" column 
# In[33]:
...
# I analysed by country, how many women with higher or no education suffer from violence. I wanted to know if women’s education has or hasn’t a relation with them suffering from violence. # Filtering the Data Frame for "No education" and "Higher" in the column “Demographics Responses” 
# In[36]:
...
# I analysed if employment status also makes a difference when the subject is violence against women # Filtering the Data Frame for the desired “Demographics Responses” 
# In[39]:
...
# For my next analysis, I need the full list of countries in the column "Country" # Extracting the list of unique countries from the "Country" column 
# In[42]:
...
#My list has 79 countries, which I divided them by religion. I wanted to separate those countries by the most known religions, to have an overview if there’s relation between religion and violence against women. # Lists of countries by religious groups 
# In[44]:
...
# My next analysis is to know if there is relation between violence against women compared to their marital status. # List of desired "Demographics Response" categories 
# In[53]:
...
# In this code, we use the isin() function to filter the original “violence_sorted” Data Frame to include only rows with "Demographics Response" values that match the desired categories. We then create a bar plot using seaborn to compare the "Value" for the specified "Demographics Response" categories.# Now my intention is to merge my violence against women Data Frame with my pregnancy and contraceptive Data Frame to analyse them together. I will merge them with how='inner', this means that the merge() function will only keep the rows where there is a matching "Country" value in both the "violence_sorted" and "contracept_sorted" data frames. Any rows with non-matching "Country" values will be dropped from the resulting merged_df data frame. 
# In[56]:
...
# Here I will start my merged analysis. I wanted to know, if religion impacts the intention of pregnancy in women.  # This code will create a bar plot using seaborn that shows the counts of "Not wanting to avoid pregnancy" and "Wanting to avoid pregnancy" for each religion. The x-axis represents different religions, the y-axis represents the count, and different colors represent the two pregnancy intention categories.  # Counting the occurrences of each combination of Pregnancy intention and Religion 
# In[60]:
...
#To finish my analysis, I wanted to analyse by religion, if the contraceptive needs of women were or weren’t met.  # Counting the occurrences of each combination of Contraceptive availability and Religion 
# In[64]:
